...
import { searchTalents } from "@/lib/talents";
...
interface TalentPageSearchParams {
  q?: string;
  companyId?: string;
  gender?: string;
  status?: string;
...
function pageHref(searchParams: TalentPageSearchParams, page: number): string {
...
export default async function TalentListPage({ searchParams }: { searchParams: TalentPageSearchParams }) {
  const [{ talents, total, page, totalPages }, companies] = await Promise.all([
    searchTalents(searchParams),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link href="/talents/new" className="btn-primary">
          ＋タレントを登録
        </Link>
      </div>
      ...
      <form className="card grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="flex gap-2 lg:col-span-2">
          ...
          <Link href="/talents" className="btn-secondary whitespace-nowrap">
            条件をクリア
          </Link>
        </div>
      </form>
...
function PagerLink({
  ...
  searchParams: TalentPageSearchParams;
  page: number;
  disabled?: boolean;
