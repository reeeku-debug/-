...
function talentInclude(yearMonth: string) {
  return {
    company: true,
    bankAccount: true,
...
export type TalentWithRelations = Prisma.TalentGetPayload<{
  include: ReturnType<typeof talentInclude>;
}>;
...
export function currentMonthPaymentRecord(talent: Pick<TalentWithRelations, "paymentRecords">) {
...
export async function searchTalents(params: TalentSearchParams): Promise<TalentSearchResult> {
  ...
  const [talents, total] = await Promise.all([
    prisma.talent.findMany({
      where,
      include: talentInclude(yearMonth),
      orderBy: { talentNo: "asc" },
      skip: (page - 1) * TALENT_PAGE_SIZE,
      take: TALENT_PAGE_SIZE,
    }),
    prisma.talent.count({ where }),
  ]);

  return { talents, total, page, totalPages: Math.max(1, Math.ceil(total / TALENT_PAGE_SIZE)) };
}
